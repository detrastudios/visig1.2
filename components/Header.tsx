...
export const Header: React.FC<HeaderProps> = ({ isDark, toggleTheme, activeTab, setActiveTab }) => {
  ...
  const handleManageKey = async () => {
    ...
    if (window.aistudio && typeof window.aistudio.openSelectKey === 'function') {
      ...
      alert("Fitur manajemen kunci hanya tersedia di lingkungan AI Studio.");
    }
  };
    ...
    <header className="sticky top-0 z-40 w-full backdrop-blur-md bg-white/80 dark:bg-slate-950/80 border-b border-gray-200 dark:border-slate-800 transition-colors">
      <div className="container mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-4">
          ...
          <div className="flex items-center gap-1.5 px-4 py-2 opacity-40 cursor-not-allowed select-none group relative">
            <svg className="w-4 h-4 text-amber-500" fill="currentColor" viewBox="0 0 20 20">
              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
            </svg>
            <span className="text-sm font-bold text-slate-500">Premium</span>
            <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[10px] py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">Coming Soon</div>
          </div>
        </nav>
            ...
            <div className="absolute right-0 top-14 w-64 bg-white dark:bg-slate-900 rounded-[1.5rem] shadow-2xl border border-gray-100 dark:border-slate-800 p-2 animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
              <div className="p-3">
                ...
                <a 
                  ...
                  className="w-full flex items-center gap-3 p-3 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors text-sm font-bold text-slate-700 dark:text-slate-200 mt-1"
                >
                  <div className="p-1.5 bg-slate-100 dark:bg-slate-800 rounded-lg">
                    <svg className="w-4 h-4 text-slate-600 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                  </div>
                  Pusat Bantuan
                </a>
              </div>
              <div className="bg-slate-50 dark:bg-slate-800/50 p-4 mt-1">
                 ...
                 <p className="text-[9px] text-slate-400 dark:text-slate-600 mt-1">Powered by Detra Studios & Gemini AI</p>
              </div>
            </div>
          )}
        </div>
